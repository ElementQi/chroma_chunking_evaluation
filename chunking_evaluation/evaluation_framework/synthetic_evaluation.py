...
class QwenSyntheticEvaluation(SyntheticEvaluation):
    def __init__(self, corpora_paths: List[str], queries_csv_path: str, chroma_db_path:str = None, openai_api_key=None):
        super().__init__(corpora_paths=corpora_paths, queries_csv_path=queries_csv_path, chroma_db_path=chroma_db_path, openai_api_key=openai_api_key)
        self.client = OpenAI(api_key=openai_api_key,
                             base_url="https://one-api.s.metames.cn:38443/v1"
                             )
        self.model_list = ["Qwen2-7B-Instruct-GPTQ-Int8", "Qwen2-72B-Instruct-GPTQ-Int8", "Meta-Llama-3.1-70B-Instruct"]
        self.embed_list = ["bce-embedding-base_v1", "zpoint_large_embedding_zh", "gte-Qwen2-7B-instruct"]
        self.model = self.model_list[2]
        self.embed = self.embed_list[2]
    ...
    def _extract_question_and_approx_references(self, corpus, document_length=4000, prev_questions=[]):
        ...
        content_1 = f"""{self.question_maker_approx_system_prompt}"""
        content_2 = f"""{self.question_maker_approx_user_prompt.replace("{document}", tagged_text).replace("{prev_questions_str}", prev_questions_str)}"""
